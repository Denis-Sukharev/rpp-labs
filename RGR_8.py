# Реализовать функцию определения риска инвестирования в определенную ценную бумагу за период анализа.
# Примечания – принимается период анализа в 30 дней из выборки, представленной в API https://www.alphavantage.co/.
# Для определения риска инвестирования в определенную ценную бумагу:
# 	 использовать функцию «TIME_SERIES_DAILY_ADJUSTED» API (https://www.alphavantage.co/query?function=TIME_SERIES_DAILY_ADJUSTED&symbol=AAPL&apikey={api_key});
# 	 вычислить ежедневную доходность и стандартное отклонение ежедневной доходности ценной бумаги за период анализа используя numpy.std.

# Ежедневная доходность рассчитывается по формуле:
#   (p_i-p_i-1)/p_i-1
# где p_i – цена закрытия ценной бумаги за день, p_i-1 – цена закрытия ценной бумаги за предыдущий день
...
os.environ['API_TOKEN'] = 'ВВЕДИТЕ СВОЙ ТОКЕН'
